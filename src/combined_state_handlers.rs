// Combined state handler implementations for axum
// This file contains updated handlers that work with the combined state tuple
...
// Copy of the OutputFormat enum from handlers.rs to make combined_state_handlers more independent
...
fn decode_input_image(
    ...
    match content_type_str {
       ...
       Some(ct) if ct.starts_with("image/x-raw-bitmap") => {
           ...
           let pixel_data_payload = file_data[16..].to_vec();
           Ok((pixel_data_payload, width, height, header_channels_byte, plugin_color_format))
       }
       _ => Err(AppError::BadRequest(format!(
           "Unsupported input image Content-Type: '{}'. Supported types are image/jpeg, image/png, image/webp, image/x-raw-bitmap.",
           content_type_str.unwrap_or("Not specified")
       ))),
   }
}

// Helper function to encode the raw pixel data (output from plugin) into the format
// requested by the client via the Accept header.
    ...
    let preferred_output_format = accept_header
        ...
        .ok_or_else(|| {
            AppError::BadRequest(format!(
                "No supported output format found in Accept header: {:?}",
                accept_header
            ))
        })?;
